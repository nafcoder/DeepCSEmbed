...
class TransformerBlock(nn.Module):
    def __init__(self, d_model=21, nhead=3, num_layers=2, dim_feedforward=64, dropout=0.1):
...
class FinalModel(nn.Module):
    def __init__(self, input_size, dropout_rate=0.3, embedding_dim=21):
        ...
        # Transformer branch for sequence data
        self.transformer_block = TransformerBlock(d_model=21, nhead=7, num_layers=2, dim_feedforward=64, dropout=0.1)
        self.pool = nn.MaxPool1d(kernel_size=2)
        self.flatten = nn.Flatten()

        # Fully connected layers
        self.transformer_fc1 = nn.Linear(15 * 10, 256)
        self.transformer_bn1 = nn.BatchNorm1d(256)
        self.transformer_relu1 = nn.ReLU()
        self.transformer_dropout1 = nn.Dropout(0.5)
    ...
    def forward(self, x):
        transformer_x = self.embedding(x[:, -15:].int())
        transformer_x = self.transformer_block(transformer_x)
        transformer_x = self.pool(transformer_x)
        transformer_x = self.flatten(transformer_x)
        transformer_x = self.transformer_fc1(transformer_x)
        transformer_x = self.transformer_bn1(transformer_x)
        transformer_x = self.transformer_relu1(transformer_x)
        transformer_x = self.transformer_dropout1(transformer_x)
        ...
        x = torch.cat((transformer_x, fnn_x), dim=1)
...
for epoch in range(31):
    for i, (inputs, labels) in enumerate(train_dataloader):
        ...
        with torch.no_grad():
            print(f'Epoch [{epoch + 1}/{31}], Step [{i + 1}/{len(train_dataloader)}]')


print("Training finished.")

file = f'Model_transformer.pth'
